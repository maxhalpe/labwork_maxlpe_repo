...
```{r}
clean_sepdx_data <- ymd_hms(se_pdx_data$date)
...
com_se_pdx_data %>%
  ggplot(aes(x = date, y = value, color = datatype)) + geom_line()
...
R1_table %>%
  ...
  coord_flip()
```
Many Reedies pursue industry and further education after Reed.
...
gradratetable <- grad_rate_table %>%
  ...
  mutate(cohort = parse_number(Cohort_size)) %>%
  mutate(four = parse_number(gradfour)) %>%
  mutate(five = parse_number(gradfive)) %>%
  mutate(six = parse_number(gradsix)) %>%
  select(Grad_year, cohort, four, five, six)
...
```
The graduation rate has improved over time for all categories.
...
#oil_scraping_table <- class(oil_scraping[[1]])
  #html_table(oil_scraping[[1]], fill = TRUE)
...
oil_csv %>%
  ggplot(mapping = aes(x = reorder(Country x = Country, y = Oil_pr)) + 
           geom_bar(stat = "identity") +
           theme_bw() + 
           labs(title = "Oil export by country", x = "Country", y = "") +
